...
type AnnotationVerdict =
  ...
  | Invalid // Not valid
type TypeOrMember = Type of TypeInfo | Member of MemberInfo
  with
  member this.CategoryString = match this with Type _ -> "type" | Member _ -> "member"
let validCsForP4 (p4:p4Attribute) (cs:TypeOrMember) : (AnnotationVerdict * compilerError[]) = // NOTE this doesn't check signatures etc.; maybe rename to avoid confustion
...
let findAnnotated (dll:DllInfo) =
  ...
  // FIXME need to resolve each key to a single type/member. There should only be a single class/intf where expected, but there is likely a class AND an intf because of gen+model
  let p4Map =
    types
    |> Seq.map (fun ty ->
        ...
        (key, value))
    |> Seq.append (members |> Seq.map (fun m ->
        let attr = m.GetCustomAttribute<p4Attribute>()
        let key = (attr.Type, attr.P4Path)
        let value = sprintf "%s.%s" m.DeclaringType.FullName m.Name
        (key, value)))
    |> Map.ofSeq
  // FIXME could check if lookup attr match kind actually matches a valid match_kind declared in this dll?
  let lookupMap =
    lookups
    |> Seq.map (fun lu ->
