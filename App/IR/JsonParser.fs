...
#if !INTERACTIVE
module P4ToCSharp.App.GenTypesList
#endif
...
let txt = """type Node(node_id, node_type) =
  interface INode
...
let classes =
  ...
  |> Seq.map (fun s -> match regexMatches "^type (?<class>[^\(\< ]*)" s with x::xs -> [x] | [] -> [])
  |> Seq.concat
  |> Seq.filter (not << System.String.IsNullOrWhiteSpace)
  |> Seq.map (fun cl -> sprintf """("%s", typeof<%s>);""" cl cl)
System.IO.File.WriteAllLines("/working/part-ii-project/classes.txt", classes)